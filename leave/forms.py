...
from .models import LeaveCategory, Leave, LeaveStep, LeaveComment, LeaveSetting
...
class LeaveCreateForm(forms.ModelForm):
    ...
    class Meta:
        model = Leave
        fields = ['title', 'description', 'category', 'priority', 'start_date', 'end_date', 'days_requested']
        widgets = {
            'title': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Leave request title'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.request:
            ...
            if tenant:
                ...
                self.fields['category'].queryset = LeaveCategory.objects.filter(tenant=tenant)
            else:
                ...
                self.fields['category'].queryset = LeaveCategory.objects.filter(tenant=None, created_by=user)


class LeaveForwardForm(forms.Form):
    """Form for forwarding / routing a leave to the next person."""
    ACTION_CHOICES = [
        ('forwarded', 'Minute'),
        ('approved', 'Approve'),
        ('rejected', 'Reject'),
...
class LeaveExternalSubmissionForm(forms.Form):
    """Public form for external leave submission."""
    ...
    category = forms.ModelChoiceField(
        queryset=LeaveCategory.objects.none(),
        required=False,
        widget=forms.Select(attrs={'class': 'form-control'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.tenant:
            self.fields['category'].queryset = LeaveCategory.objects.filter(tenant=self.tenant)
...
class LeaveSettingForm(forms.ModelForm):
    class Meta:
        model = LeaveSetting
